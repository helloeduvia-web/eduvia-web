...
function Home({go,openAssessment}){return <main>
  <section className="hero"><div className="container hero-grid"><div className="hero-copy"><span className="eyebrow">GUIDED GLOBAL STUDY ABROAD PLATFORM</span><h1>Make your study-abroad journey <em>clearer.</em></h1><p>Discover the right destination, course and next step through one guided journey built around your profile.</p><div className="hero-buttons"><button className="primary" onClick={openAssessment}>Start Free Assessment <Icon name="arrow"/></button><button className="secondary" onClick={()=>go('destinations')}>Explore Destinations</button></div><div className="trust-row"><span><Icon name="check"/> 5 focused questions</span><span><Icon name="check"/> Immediate value</span><span><Icon name="check"/> Personalised pathway</span></div></div><div className="hero-media"><img className="hero-photo" src="/assets/global-hands.jpg" alt="Students reaching toward the sky together"/><img className="hero-logo-overlay" src="/assets/eduvia-wordmark-transparent.png" alt="Eduvia — Delivering Opportunities"/><div className="float-card"><span>Profile readiness</span><strong>82 <small>/ 100</small></strong><div className="mini-bar"><i/></div></div></div></div></section>
  ...
  <section className="section destination-preview"><div className="container"><div className="section-row"><div><span className="eyebrow">EXPLORE DESTINATIONS</span><h2>One platform. <em>Multiple destinations.</em></h2><p>Not sure? Let the platform recommend the best-fit destinations.</p></div><button className="secondary" onClick={()=>go('destinations')}>Explore all destinations <Icon name="arrow"/></button></div><div className="destination-grid">{destinations.map(([c,t,i])=><div className="destination-card" key={c}><div className="country-icon">{i}</div><div><strong>{c}</strong><span>{t}</span></div><Icon name="arrow"/></div>)}</div></div></section>
  <section className="section global-story"><div className="container"><div className="section-row"><div><span className="eyebrow">THE EDUVIA JOURNEY</span><h2>See the journey. <em>Understand the next step.</em></h2><p>Real-world context helps you compare destinations and picture what your international study journey can look like.</p></div></div><div className="story-grid student-story-grid"><article className="story-card story-wide"><img src="/assets/global-community-map.jpg" alt="Students gathered around a world map"/><div><span>GLOBAL COMMUNITY</span><h3>Think beyond borders.</h3><p>Explore destinations, people and opportunities around the world from one student-first platform.</p></div></article><article className="story-card"><img src="/assets/global-students-flags.jpg" alt="International students representing different countries"/><div><span>DESTINATIONS</span><h3>Find your place in the world.</h3><p>Compare countries with a clearer view of fit, timing and pathways.</p></div></article></div></div></section>
...
function Intakes({openAssessment}){
  ...
  return <main className="inner-page intake-page">
    ...
    </div><div className="inner-hero-image"><img src="/assets/student-finance.jpg" alt="Students preparing for international study"/></div></div></section>
...
function Result({answers,onClose}){const destination=answers[4]||'Not sure — recommend for me'; return <div className="result"><div className="result-score"><span>FREE PROFILE SNAPSHOT</span><strong>82</strong><small>/ 100</small><p>Starting readiness</p></div><div><span className="eyebrow">PRACTICAL PROFILE SNAPSHOT</span><h2>We’ve got a useful starting point.</h2><p>Your answers give Eduvia enough context to explain what to work on next — without pretending that one score can guarantee admission or a visa.</p><div className="result-grid"><div><b>Study stage</b><span>{answers[0]||'Not added yet'}</span></div><div><b>Career direction</b><span>{answers[1]||'Not added yet'}</span></div><div><b>First-year budget</b><span>{answers[2]||'Not added yet'}</span></div><div><b>Preferred region</b><span>{destination}</span></div></div><div className="practical-next"><b>What to do next</b><span>1. Complete your profile and English details.</span><span>2. Compare destinations against your realistic budget.</span><span>3. Shortlist courses before speaking with a counsellor.</span></div><div className="result-actions"><button className="ghost" onClick={onClose}>Continue exploring</button><button className="primary" onClick={onClose}>Open My Student Journey <Icon name="arrow"/></button></div></div></div>}
